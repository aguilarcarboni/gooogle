...
{
    // Define a set of common stop words to be excluded from processing
    public static HashSet<string> StopWords = new HashSet<string>
    {
        "a", "an", "and", "are", "as", "at", "be", "by", "for", "from",
        "has", "he", "in", "is", "it", "its", "of", "on", "that", "the",
        "to", "was", "were", "will", "with", "what"
    };

    private static EnglishPorter2Stemmer stemmer = new EnglishPorter2Stemmer();

    // Stemmer function using Porter2Stemmer
    private static string StemWord(string word)
    {
        return stemmer.Stem(word).Value;
    }

    // Convert all documents into a single list of words
    ...
    // Create a dictionary of word lists for each document
    public static Dictionary<string, List<string>> CreateWordLists(Dictionary<string, string> documents)
    {
        // Dictionary to collect lists of words from each document
    ...
    // Generate a list of unique stemmed words from all documents, excluding stop words
    public static List<string> Unique_Words(List<string> documents)
    {
        ...
        foreach (string document in documents)
        {
            ...
            // Add each stemmed word to the HashSet, excluding stop words and very short words
            foreach (string word in words)
            {
                string lowercaseWord = word.ToLower();
                if (!StopWords.Contains(lowercaseWord) && lowercaseWord.Length > 1)
                {
                    string stemmedWord = StemWord(lowercaseWord);
        ...
        // Return the list of unique stemmed words
        return myList;
    }
    ...
    public static Dictionary<string, Dictionary<string, int>> CountWordOccurrences(Dictionary<string, List<string>> docs, List<string> allUniqueWords)
    {
        ...
        foreach (var doc in docs)
        {
            ...
            foreach (string word in wordList)
            {
                ...
                // If the stemmed word is in the dictionary, increment its count
